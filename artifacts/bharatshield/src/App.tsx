...
import { AlertTriangle, ArrowRight, AudioLines, Check, ChevronRight, CircleHelp, Eye, FileAudio, FileImage, FileVideo, Fingerprint, Gauge, GitBranch, History as HistoryIcon, Info, Languages, ListChecks, Lock, Menu, MessageSquareText, Play, Plus, Radar, ScanSearch, ShieldAlert, ShieldCheck, Sparkles, Trash2, Upload, X, Zap } from 'lucide-react';
...
function Shell({ lang, setLang, children }: { lang: Lang; setLang: (v: Lang) => void; children: React.ReactNode }) {
  ...
  return <div className="app-shell">
    <header className="topbar">
      <Link href="/" className="brand" data-testid="link-brand"><span className="brand-mark"><ShieldCheck size={18} /></span><span className="brand-word">Fake<em>Trace</em></span></Link>
...
const flow = [
  ...
  { no: 'STEP 02', title: 'Analysis', body: 'FakeTrace reads the available content and metadata for known patterns.' },
...
const faqs = [
  { q: 'Does FakeTrace guarantee a message is a scam?', a: 'No. It gives a first-pass risk assessment and names the signals behind it. Final judgment always stays with you and, where money is involved, with your bank’s fraud team.' },
  ...
  { q: 'Is my data kept private?', a: 'FakeTrace is built as a first-pass companion. Do not upload sensitive documents you do not need to check, and treat every result as guidance rather than a verdict.' },
];
...
function Home({ lang }: { lang: Lang }) {
  ...
  return <main>
    <section className="hero">
      ...
      <div className="signal-board" aria-label="Illustrative FakeTrace assessment — not a real analysis">
        <div className="board-card board-main">
          <div className="board-label"><span>FAKETRACE / FIRST PASS</span><span className="mono">ILLUSTRATIVE</span></div>
    ...
    <section className="section" id="why">
      <div className="section-head"><div className="eyebrow">Why FakeTrace</div><h2>Scams rarely arrive looking like scams.</h2><p>Suspicious digital content, misinformation and manipulation are engineered to feel ordinary. FakeTrace helps you slow down and see the signals.</p></div>
    ...
    <section className="section section-alt" id="multimodal">
      <div className="section-head"><div className="section-kicker">Multimodal Intelligence</div><h2>One assessment across four kinds of content.</h2><p>Threats do not stay in one format. FakeTrace is designed to reason across text, audio, image and video signals.</p></div>
    ...
    <section className="section" id="how">
      <div className="section-head"><div className="eyebrow">How FakeTrace works</div><h2>Five calm steps from a strange message to a safer decision.</h2></div>
    ...
    <section className="section section-alt" id="explainable">
      <div className="split">
        <div>
          ...
          <p>Black-box verdicts are easy to distrust and easy to ignore. FakeTrace shows the understandable signals behind every assessment, so you can judge them yourself.</p>
    ...
    <section className="section" id="faq">
      <div className="section-head"><div className="eyebrow">Questions</div><h2>What FakeTrace does — and does not — claim.</h2></div>
    ...
    <footer className="home-footer">
      <div className="footer-inner">
        <div className="footer-brand"><div className="brand"><span className="brand-mark"><ShieldCheck size={18} /></span><span className="brand-word">Fake<em>Trace</em></span></div><p>{t.footer} FakeTrace is a first-pass companion, not a replacement for your judgment or your bank’s fraud team.</p></div>
        ...
        <div className="footer-col"><h4>Company</h4><Link href="/about">About</Link><a href="#why">Why FakeTrace</a><a href="#how">How it works</a><a href="#faq">FAQ</a></div>
      ...
      <div className="footer-bar"><div className="footer-bar-inner"><span>© {new Date().getFullYear()} FakeTrace · A clearer pause in a noisy inbox.</span><span className="mono">Prototype · Not financial advice</span></div></div>
    </footer>
  </main>;
}

function riskDisplay(level?: string) { const l = (level || 'MEDIUM').toUpperCase(); return l === 'MEDIUM' ? 'MODERATE' : l; }
function RiskBadge({ level }: { level?: string }) { return <span className={`risk-badge risk-${(level || 'medium').toLowerCase()}`}>{riskDisplay(level)} RISK</span>; }

function EvidenceBreakdown({ items }: { items: EvidenceItem[] }) {
  if (!items.length) return <p className="explanation">No individual risk signals were detected.</p>;
  return <ul className="evidence-list">{items.map((e, i) => <li key={i} className={`evidence-item sev-${e.severity}`} data-testid={`evidence-${i}`}>
    <div className="evidence-top"><span className="evidence-label">{e.label}</span><span className={`sev-tag sev-${e.severity}`}>{e.severity} · +{e.points}</span></div>
    <p className="evidence-reason">{e.reason}</p>
    {e.matched && <span className="evidence-matched">matched: “{e.matched}”</span>}
  </li>)}</ul>;
}

function ClaimVerification({ claims }: { claims: ClaimItem[] }) {
  return <ul className="claim-list">{claims.map((c, i) => <li key={i} className="claim-item" data-testid={`claim-${i}`}>
    <div className="claim-top"><span className={`claim-tag claim-${c.status.toLowerCase()}`}>{c.status}</span></div>
    <p className="claim-text">“{c.text}”</p>
    <span className="claim-note">{c.note}</span>
  </li>)}</ul>;
}

function ManipulationFlow({ stages }: { stages: ManipulationStage[] }) {
  return <div className="intent-flow">{stages.map((s, i) => <div key={s.stage} className={`intent-stage ${s.present ? 'on' : 'off'}`} data-testid={`intent-${s.stage}`}>
    <span className="intent-name">{s.present ? <Check size={13} /> : <X size={13} />} {s.stage}</span>
    <span className="intent-detail">{s.detail}</span>
    {i < stages.length - 1 && <ChevronRight className="intent-arrow" size={14} />}
  </div>)}</div>;
}

function ResultCard({ result }: { result: AnalysisResult }) {
  const [saved, setSaved] = useState(false);
  return <div className="panel panel-pad"><div className="risk-header"><div><div className="eyebrow">{result.demo ? 'DEMO MODE · TRUST REPORT' : 'FAKETRACE TRUST REPORT'}</div><div className="risk-score" data-testid="text-result-score">{result.score}<small> / 100</small></div></div><RiskBadge level={result.riskLevel} /></div>

    <div className="report-meta-grid">
      <div className="rm-cell"><span className="rm-k"><Gauge size={13} /> Confidence</span><span className="rm-v">{result.confidenceLabel ?? '—'}{typeof result.confidence === 'number' ? ` (${result.confidence}%)` : ''}</span></div>
      <div className="rm-cell"><span className="rm-k"><ShieldAlert size={13} /> Risk level</span><span className="rm-v">{riskDisplay(result.riskLevel)}</span></div>
    </div>

    {result.threatTypes && result.threatTypes.length > 0 && <div className="result-section"><h3>Threat types</h3><div className="chip-row">{result.threatTypes.map((tt, i) => <span key={i} className="threat-chip" data-testid={`threat-type-${i}`}>{tt}</span>)}</div></div>}
    ...
    {result.evidence && <div className="result-section"><h3><ListChecks size={13} className="sec-ic" /> Evidence breakdown</h3><EvidenceBreakdown items={result.evidence} /></div>}

    {result.manipulationIntent && <div className="result-section"><h3><GitBranch size={13} className="sec-ic" /> Manipulation intent</h3><ManipulationFlow stages={result.manipulationIntent} /></div>}

    {result.claims && <div className="result-section"><h3><ScanSearch size={13} className="sec-ic" /> Claim verification</h3><ClaimVerification claims={result.claims} /></div>}

    {(result.provenance || result.mediaAuthenticity) && <div className="result-section"><h3><Fingerprint size={13} className="sec-ic" /> Provenance &amp; media authenticity</h3>
      {result.provenance && <div className="prov-row"><span className="prov-status">{result.provenance.status}</span><p className="prov-detail">{result.provenance.detail}</p></div>}
      {result.mediaAuthenticity && <div className="prov-row"><span className="prov-status prov-unavailable">Media authenticity: {result.mediaAuthenticity.status}</span><p className="prov-detail">{result.mediaAuthenticity.detail}</p></div>}
    </div>}

    {result.beforeYouShare && <div className="result-section"><h3><AlertTriangle size={13} className="sec-ic" /> Before you share</h3><p className="before-share">{result.beforeYouShare}</p></div>}

    <div className="result-section"><h3>What to do next</h3><ul className="recommend-list">{result.recommendations.map((r, i) => <li key={i}>{r}</li>)}</ul></div>

    {result.limitations && result.limitations.length > 0 && <div className="result-section"><h3><Info size={13} className="sec-ic" /> Limitations</h3><ul className="limit-list">{result.limitations.map((l, i) => <li key={i}>{l}</li>)}</ul></div>}

    <div className="result-meta"><span className="meta-tag">{result.contentType}</span><span className="meta-tag">{result.language}</span><span className="meta-tag">deterministic engine</span></div>{result.id && !result.demo && <div className="result-actions"><button className="btn btn-quiet" onClick={() => setSaved(true)} disabled={saved} data-testid="button-save-analysis">{saved ? 'Saved to history' : 'Save Analysis'} <Check size={15} /></button><Link href="/history" className="btn btn-primary" data-testid="link-view-saved-analysis">View Report <ChevronRight size={15} /></Link></div>}</div>;
}
...
function Analyze({ lang }: { lang: Lang }) {
  ...
  return <main className="page"><div className="workspace-head"><div><div className="eyebrow">THE CHECKPOINT</div><h1 className="page-title">Make the pause<br />count.</h1><p className="page-lead">Bring the thing that made you hesitate. FakeTrace looks for familiar warning patterns and tells you how to verify them.</p></div><Link href="/about" className="btn btn-outline" data-testid="link-analysis-limits"><CircleHelp size={15} /> How this works</Link></div>
    ...
    <div className="notice" style={{ marginTop: 18 }}><Info size={15} /> Media note: this build analyses file metadata and the file name only — content is not transcribed or OCR-scanned, and AI-generated/deepfake detection is shown as Unavailable rather than guessed. Never upload sensitive documents you do not need to check.</div>
  </main>;
}
...
const demos = [
  { title:'The urgent KYC', desc:'Urgency + payment + threat + link', message:'URGENT: Your KYC will expire in 30 minutes. Pay ₹2 now via bit.ly/kyc-verify to keep your UPI active, or your bank account will be blocked today.' },
  { title:'The lucky winner', desc:'Prize lure + credential request', message:'Congratulations! You have won a ₹25,00,000 lottery prize. Share your bank account number and OTP to claim your reward today.' },
  { title:'The fake bank support', desc:'Impersonation + credentials + contact', message:'This is SBI customer care. Your account is suspended for security reasons. Verify your net banking password immediately by calling this number 9876543210.' },
  { title:'The refund trap', desc:'Impersonation + suspicious domain', message:'Income Tax refund of ₹15,240 is pending. Update your card details at http://income-tax-refund.xyz within 24 hours to receive the amount.' },
  { title:'The normal message', desc:'Low-signal conversation', message:'Hi, are we still meeting for the project review next Tuesday at 4 pm? Let me know if that time works for you.' },
];
function DemoPage() {
  const [index, setIndex] = useState(0);
  const d = demos[index];
  const report = useMemo(() => analyzeMessage({ text: d.message, demo: true }), [d.message]);
  return <main className="page"><div className="eyebrow">DEMO MODE · FICTIONAL TEST CASES</div><h1 className="page-title">Five messages.<br />Five real readouts.</h1><p className="page-lead">A guided lab that runs five fictional scenarios through the exact same FakeTrace threat engine used for real checks. The scores below are live, deterministic outputs — not scripted. These messages are invented for testing and impersonate no real person.</p>
    <div className="notice" style={{ marginBottom:20 }}><Sparkles size={15} /> DEMO MODE — every scenario is fictional and generated for demonstration. No real individuals are impersonated.</div>
    <div className="demo-grid">{demos.map((x,i) => <button key={x.title} className={`demo-card ${i === index ? 'selected' : ''}`} onClick={() => setIndex(i)} data-testid={`button-demo-scenario-${i+1}`}><span className="demo-no">DEMO 0{i+1}</span><h3>{x.title}</h3><p>{x.desc}</p></button>)}</div>
    <div className="demo-layout"><section className="demo-script"><div className="eyebrow">DEMO 0{index+1} · FICTIONAL</div><h2>{d.title}</h2><p>{d.desc}. This runs through the live engine so you can see exactly which signals drive the score.</p><div className="demo-message">“{d.message}”</div><Link href="/analyze" className="btn btn-quiet" data-testid="link-demo-real-check">Try a real check <ArrowRight size={15} /></Link></section>
    <section><ResultCard result={report} /></section></div>
  </main>;
}
